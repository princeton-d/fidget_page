...
function handleWelcomeInput(e) {
  // 처음 화면에 text를 submit 하면 컨텐츠박스를 노출시킴
...
export function toggleDisplay(target, state) {
  // display 컨트롤
  target.style.display = state;
}
function makeCursorText(cursorText) {
  // 커서 텍스트를 화면에 보여줌
...
function mouseMove(e) {
  ...
  cursor.style.top = `${e.pageY + 1.8}px`;
}
// event handling
